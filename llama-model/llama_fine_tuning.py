...
def main():
    ...
    model_name = os.path.join(_script_dir, "llama-3.2-1B-Instruct")
    output_dir = os.path.join(_script_dir, "llama-3.2-1B-Instruct-vuln-lora")
    ...
    # 데이터셋 로드 및 SFT용 전처리
    # =========================
    _data_path = os.path.join(_script_dir, "data", "secure_programming_dpo_flat.json")
    print(f"1) 데이터셋 로드: {_data_path} ...")
    dataset = load_dataset('json', data_files=_data_path)
    ...
    training_arguments = SFTConfig(
        ...
        learning_rate=2e-4,
        warmup_ratio=0.03,
        dataloader_num_workers=2,   # macOS/Linux; set to 0 on Windows
        remove_unused_columns=False,
        packing=True,               # macOS/Linux; set to False on Windows
    ...
    _merged_dir = os.path.join(_script_dir, "merged-vuln-detector")
    merged.save_pretrained(_merged_dir, safe_serialization=True)
